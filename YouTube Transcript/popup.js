...
let devanagariFontBase64Promise = null;

function containsDevanagari(text) {
  return DEVANAGARI_RE.test(String(text || ''));
}

async function getDevanagariFontBase64() {
  if (!devanagariFontBase64Promise) {
    devanagariFontBase64Promise = (async () => {
      const url = chrome.runtime.getURL(DEVANAGARI_FONT.file);
      const res = await fetch(url);
      if (!res.ok) {
        throw new Error('Could not load Devanagari font file.');
      }
  ...
  return devanagariFontBase64Promise;
}

async function ensureDevanagariFont(doc) {
  const fontList = typeof doc.getFontList === 'function' ? doc.getFontList() : null;
  if (fontList && Object.prototype.hasOwnProperty.call(fontList, DEVANAGARI_FONT.family)) {
    return;
  }

  const base64 = await getDevanagariFontBase64();
  doc.addFileToVFS(DEVANAGARI_FONT.vfsName, base64);
  doc.addFont(DEVANAGARI_FONT.vfsName, DEVANAGARI_FONT.family, 'normal');
}
...
async function buildPDF(title, blocks) {
  ...
  const needsDevanagari = [title, ...blocks.map((b) => b?.text || '')].some(containsDevanagari);
  let devanagariFontReady = false;

  if (needsDevanagari) {
    try {
      await ensureDevanagariFont(doc);
      devanagariFontReady = true;
    } catch (error) {
      console.warn('[YT-PDF] Could not load Devanagari font; falling back to default font.', error);
    }
  }
  ...
  function renderBlock(type, text) {
    ...
    const useDevanagariFont = devanagariFontReady && containsDevanagari(text);

    doc.setFontSize(s.size);
    doc.setFont(useDevanagariFont ? DEVANAGARI_FONT.family : 'helvetica', useDevanagariFont ? 'normal' : s.style);
